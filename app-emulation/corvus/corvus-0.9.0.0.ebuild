...
src_compile() {
	...
	if use python; then
		...
		# the tarball install picks up the current Request/Response
		# surface rather than whatever was committed.
		local genbin
...
_corvus_install_python_module() {
	# Install the corvus_client package into the current impl's
	# site-packages. python_domodule handles the directory recursively.
